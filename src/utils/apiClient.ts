...
export class ApiClient {
  ...
  configure(config: ApiClientConfig = {}): void {
    ...
    if (config.retry !== undefined)
      this.retryOptions = { ...this.retryOptions, ...config.retry };
  }
...
export async function fetchPublishedByCode(
  code: string,
...
export async function fetchPublishedByPublishId(
  publishId: string,
): Promise<PublishedSchemaItem | null> {
  if (apiClient.isMockEnabled()) {
...
export async function fetchPublishedSchema(
  editId: string,
): Promise<PublishedSchemaItem | null> {
  if (apiClient.isMockEnabled()) {
...
export async function createSchema(
  ...
  return apiClient.post<SchemaDetail>(
    "/schemas",
...
export async function updateSchema(
  ...
  return apiClient.put<SchemaDetail>(
...
export async function fetchDictByCode(code: string): Promise<DictItem[]> {
  return apiClient.get<DictItem[]>(
    `/dict/data/by-type/${encodeURIComponent(code)}`,
  );
}
...
export async function fetchMockData(
  ...
  return apiClient.get<Record<string, unknown>>(
    `/mock/${encodeURIComponent(schemaId)}`,
  );
}
...
export async function fetchTemplates(params?: {
  ...
  if (params?.isBuiltin !== undefined)
    query.isBuiltin = String(params.isBuiltin);
...
export async function applyTemplate(id: string): Promise<TemplateApplyResult> {
  return apiClient.post<TemplateApplyResult>(
...
export async function fetchFlowInstances(options?: {
  definitionId?: string;
  status?: string;
  page?: number;
  pageSize?: number;
  ...
  return apiClient.get<PaginatedResponse<FlowInstanceItem>>(
    "/flow-instances",
    params,
  );
}

export async function fetchFlowInstanceById(
  id: string,
): Promise<FlowInstanceItem> {
  return apiClient.get<FlowInstanceItem>(
    `/flow-instances/${encodeURIComponent(id)}`,
  );
}
