 ...
 *
 * 当 VITE_USE_MOCK=true Hrs, 替换真实 API 客户端, 
 * Back本地假Data, 无需后端服务即可Debug设计器。
 *
 * 使用方式：在 .env.development 中Settings VITE_USE_MOCK=true
 */
import type {
  SchemaListItem,
  SchemaDetail,
...
export async function mockCreateSchema(
...
export async function mockPublishSchema(
  id: string,
  version?: string,
  ...
  if (version && version !== schema.version) {
    ...
    // mock 无快照Hrs仍使用当前 json, 但保留Version号
  ...
  const published: PublishedSchemaItem = {
    ...
    type: schema.type,
    // @ts-ignore - json 格式兼容
    json: (Array.isArray(publishJson)
      ? publishJson
      : (publishJson?.widgets ?? [])) as unknown[],
...
export async function mockFetchPublishedSchema(
  sourceId: string,
...
export async function mockFetchPublishedByPublishId(
  publishId: string,
): Promise<PublishedSchemaItem | null> {
  await delay();
...
export async function mockFetchVersions(
  editId: string,
  page = 1,
  pageSize = 10,
): Promise<{ items: any[]; total: number }> {
  await delay();
  // 按 editId or id 查找 schema
  const schema =
    MOCK_SCHEMAS.get(editId) ??
    Array.from(MOCK_SCHEMAS.values()).find((s) => s.editId === editId);
  ...
  const allVersions = [
    {
      ...
      createdAt: schema.updatedAt || schema.createdAt,
      published: false,
    },
...
export async function mockFetchVersion(
  editId: string,
  version: string,
): Promise<any> {
  await delay();
...
export async function mockDeleteVersion(
  editId: string,
  version: string,
): Promise<null> {
  await delay();
