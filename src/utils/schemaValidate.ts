...
const NO_FIELD_TYPES = new Set<string>([
  "button",
  "toolbar-buttons",
  "title",
  "banner",
...
export type ValidationRuleType =
  ...
  | "form-id-orphan"
  | "tab-key-orphan"
  | "dialog-target-invalid"
  | "trigger-event-orphan"
  | "lifecycle-string-ignored";
...
function detectLinkageCycles(
  graph: Map<string, Set<string>>,
): { field: string; cycle: string[] }[] {
...
export function validateSchema(schema: PartialWidget[]): ValidationResult {
  ...
  function walk(items: PartialWidget[], path: number[], depth: number): void {
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.field) {
        fieldCounts.set(item.field, [
          ...(fieldCounts.get(item.field) ?? []),
          ...itemPath,
        ]);
      }
      ...
      if (
        item.field &&
        item.linkages?.some((l) => l.type === "required") &&
        !item.label
      ) {
        errors.push({
          path: itemPath,
      ...
      if (
        ...
        if (
          ...
          item.defaultValue !== "" &&
          isNaN(Number(item.defaultValue))
        ) {
          errors.push({
            path: itemPath,
  ...
  function collectGlobals(items: PartialWidget[], path: number[]) {
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.type === "form" && item.id) formContainerIds.add(item.id);
      if (item.type === "dialog" && item.id) dialogIds.add(item.id);
      if (item.type === "tabs" && item.props?.tabs) {
        const keys = new Set<string>();
        for (const tab of item.props.tabs as Array<{ key?: string }>) {
          if (tab.key) keys.add(tab.key);
        }
        if (item.id) tabKeysMap.set(item.id, keys);
      }
