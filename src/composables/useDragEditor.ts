...
export function useDragEditor(
  options: UseDragEditorOptions,
): UseDragEditorReturn {
  ...
  function handleDropToContainer(
    parentPath: number[],
    index: number,
    item: PartialWidget,
  ) {
  ...
  function handleDragToContainer(
    sourcePath: number[],
    targetPath: number[],
    targetIndex: number,
  ) {
    ...
    if (targetStr.startsWith(sourceStr + ",") || targetStr === sourceStr)
      return;
