...
function createLinkageRule(
  overrides: Partial<ChartLinkageRule> = {},
): ChartLinkageRule {
  return {
...
describe("useChartLinkage", () => {
  ...
  it("handles drilldown action and maintains history", () => {
    ...
    const { handleChartClick, getBreadcrumbs, hasDrilldown, getActiveFilters } =
      useChartLinkage({
        widgets,
        updateWidget,
      });
  ...
  it("handles multiple targets for a single rule", () => {
    ...
    expect(getActiveFilters("target-2")).toEqual({
      filterCategory: "Electronics",
    });
  });
  ...
  it("clears linkage state when going back to top level", () => {
    ...
    const { handleChartClick, drilldownBack, getActiveFilters, getHighlights } =
      useChartLinkage({
        widgets,
        updateWidget,
      });
