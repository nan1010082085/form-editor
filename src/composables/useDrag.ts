...
export function useDrag() {
  ...
  function applyPercentPosition(widget: Widget) {
    ...
    if (widget.position.xUnit === "%" && canvasW > 0) {
      widget.position.x =
        Math.round((widget.position.x / canvasW) * 100 * 100) / 100;
    }
    if (widget.position.yUnit === "%" && canvasH > 0) {
      widget.position.y =
        Math.round((widget.position.y / canvasH) * 100 * 100) / 100;
    }
  }
  ...
  function findContainerCanvasOffset(containerId: string): {
    ...
    return offsets.reduce((acc, o) => ({ x: acc.x + o.x, y: acc.y + o.y }), {
      x: 0,
      y: 0,
    });
  }
  ...
  function computeDropPreviewLine(
    ...
    if (closestWidget) {
      ...
      const previewY = insertBefore ? cwp.y : cwp.y + cwp.h;
      return {
        orientation: "horizontal",
        position: previewY,
        start: 4,
  ...
  function updateDrag(clientX: number, clientY: number, canvasEl: HTMLElement) {
    ...
    if (dragStore.dragSource === "canvas" && dragStore.dragWidgetId) {
      ...
      if (w) {
        widgetW = w.position.w;
        widgetH = w.position.h;
      }
    }
    ...
    const isContainerDrag =
      draggedType && getContainerTypeSet().has(draggedType);
    ...
    if (isContainerDrag) {
      ...
      if (allContainers.length > 0) {
        ...
        const hitContainer = detectContainerCollision(
          tempWidget as Widget,
          containers,
          canvasWidth,
          canvasHeight,
      ...
      // 计算放置预览线
      const previewLine = computeDropPreviewLine(
        canvasX,
        canvasY,
        widgetW,
        widgetH,
    ...
    // 辅助线计算（画布内拖拽和面板拖入都计算）
    if (
      (dragStore.dragSource === "canvas" && dragStore.dragWidgetId) ||
      dragStore.dragSource === "panel"
    ) {
      ...
      if (dragStore.dragSource === "panel") {
        ...
        if (draggingWidget) {
          ...
          if (parentContainer) {
            ...
            const containerW =
              containerWUnit === "%" ? (canvasW * ppos.w) / 100 : ppos.w;
            const containerH =
              containerHUnit === "%" ? (canvasH * ppos.h) / 100 : ppos.h;
            const containerRect = {
              x: cx + ppos.x,
              y: cy + ppos.y,
              w: containerW,
            ...
            const { lines, snapX, snapY } = calculateContainerGuides(
              dragRect,
              siblings,
              containerRect,
            );
