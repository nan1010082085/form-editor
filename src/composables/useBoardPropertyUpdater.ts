...
export function useBoardPropertyUpdater(
  ...
  function updateBoardProperty(key: string, value: unknown) {
    ...
    if (key.startsWith("freeLayout.")) {
      const subKey = key.slice("freeLayout.".length) as keyof NonNullable<
        typeof boardStore.canvas.freeLayout
      >;
      const fl = {
    ...
    if (key === "widthUnit") {
      ...
      if (newUnit !== oldUnit) {
        ...
        if (oldUnit === "px" && newUnit === "%" && parentWidth > 0) {
          boardStore.updateCanvas({
            widthUnit: newUnit,
            width: Math.round((currentWidth / parentWidth) * 100 * 100) / 100,
          });
          return;
        } else if (oldUnit === "%" && newUnit === "px") {
          boardStore.updateCanvas({
            widthUnit: newUnit,
            width: Math.round((parentWidth * currentWidth) / 100),
          });
          return;
        }
      }
      ...
      if (newUnit !== oldUnit) {
        ...
        if (oldUnit === "px" && newUnit === "%" && parentHeight > 0) {
          boardStore.updateCanvas({
            heightUnit: newUnit,
            height:
              Math.round((currentHeight / parentHeight) * 100 * 100) / 100,
          });
          return;
        } else if (oldUnit === "%" && newUnit === "px") {
          boardStore.updateCanvas({
            heightUnit: newUnit,
            height: Math.round((parentHeight * currentHeight) / 100),
          });
          return;
        }
      }
