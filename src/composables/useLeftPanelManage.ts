...
export function useLeftPanelManage(
  options: UseLeftPanelManageOptions,
): UseLeftPanelManageReturn {
  const {
    schema,
    selectedPath,
    selectedPaths,
  ...
  function handleTreeReorder(payload: {
    ...
    if (
      sourceParent.join(",") === insertParentPath.join(",") &&
      sourceIdx < insertIndex
    ) {
      insertIndex--;
    }
