...
async function handleDelete(tenant: TenantItem) {
  try {
    ...
    else ElMessage.error(tenantStore.error || t('editor.tenantView.deleteFailed'))
  } catch {
    // 用户取消
  }
}
...
async function handleToggleStatus(tenant: TenantItem) {
  ...
  const actionLabel = newStatus === 'active' ? t('editor.tenantView.statusActive') : t('editor.tenantView.statusInactive')
  try {
    await ElMessageBox.confirm(
      t('editor.tenantView.toggleConfirm', { action: actionLabel, name: tenant.name }),
    ...
    if (result) ElMessage.success(t('editor.tenantView.toggled', { action: actionLabel }))
    else ElMessage.error(tenantStore.error || t('editor.tenantView.toggleFailed', { action: actionLabel }))
  } catch {
    // 用户取消
  }
}
  ...
  <div :class="styles.tenantView">
    <div :class="styles.scrollbar">
      ...
      <div :class="styles.header">
        <div :class="styles.titleRow">
          <div>
            ...
            <p :class="styles.subtitle">{{ t('editor.tenantView.subtitle') }}</p>
          </div>
      ...
      <div v-else-if="tenantStore.isEmpty" :class="styles.emptyState">
        ...
        <h2 :class="styles.emptyTitle">{{ t('editor.tenantView.emptyTitle') }}</h2>
      ...
      <div v-else :class="styles.tableWrapper">
        <el-table
          ...
          row-key="id"
        >
          <el-table-column prop="name" :label="t('editor.tenantView.colName')" min-width="160" show-overflow-tooltip />
          <el-table-column prop="code" :label="t('editor.tenantView.colCode')" min-width="140" show-overflow-tooltip>
          ...
          <el-table-column prop="maxUsers" :label="t('editor.tenantView.colMaxUsers')" width="100" align="center">
            <template #default="{ row }">
              {{ row.config.maxUsers }}
            </template>
          </el-table-column>
          <el-table-column prop="features" :label="t('editor.tenantView.colFeatures')" min-width="180">
            <template #default="{ row }">
          ...
          <el-table-column prop="createdAt" :label="t('editor.common.createdAt')" width="170">
            <template #default="{ row }">
              {{ formatDate(row.createdAt) }}
            </template>
          </el-table-column>
          <el-table-column :label="t('editor.common.actions')" width="200" fixed="right">
            <template #default="{ row }">
              <el-button size="small" type="primary" link @click="openEditDialog(row)">{{ t('editor.common.edit') }}</el-button>
              <el-button
                size="small"
              ...
              >
                {{ row.status === 'active' ? t('editor.tenantView.statusInactive') : t('editor.tenantView.statusActive') }}
              </el-button>
              <el-button size="small" type="danger" link @click="handleDelete(row)">{{ t('editor.common.delete') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
