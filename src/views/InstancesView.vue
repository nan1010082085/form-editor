...
function getJsonLength(json: SchemaListItem["json"]): number {
  ...
  if (json && typeof json === "object" && "widgets" in json)
    return (json.widgets as PartialWidget[]).length;
  return 0;
}
...
const filterTabs = computed(() => [
  ...
  {
    label: t("editor.instances.filterSearchList"),
    value: "search-list" as const,
  },
...
>(() => [
  ...
  },
  {
    ...
    value: "free",
    desc: t("editor.instances.layoutFreeDesc"),
  },
]);
...
const freePresetOptions = computed<
  { label: string; value: FreeLayoutPreset }[]
>(() => [
...
const selectedLayoutDesc = computed(
  () =>
    layoutModeOptions.value.find((o) => o.value === createLayoutMode.value)
      ?.desc ?? "",
);
...
async function handleBulkDelete() {
  ...
  try {
    await ElMessageBox.confirm(
      t("editor.instances.bulkDeleteMessage", {
  ...
  try {
    ...
    if (fail === 0)
      ElMessage.success(
        t("editor.instances.bulkDeleteSuccess", { count: success }),
      );
    else
      ElMessage.warning(
        t("editor.instances.bulkDeletePartial", { success, fail }),
      );
...
async function confirmImport() {
  ...
  try {
    ...
    ElMessage.error(
      err instanceof Error ? err.message : t("editor.instances.importFailed"),
    );
  } finally {
...
async function confirmEdit() {
  ...
  try {
    await updateSchema(editId.value, {
...
function handleLoadVersion(version: string) {
  ...
  if (versionDialogEditId.value) {
    router.push({
      path: "/editor",
      query: { editId: versionDialogEditId.value, version },
    });
  }
}
...
<template>
  <div :class="styles['fg-instances']">
    <div :class="styles['fg-instances__scrollbar']">
      ...
      <div :class="styles['fg-instances__header']">
        <div :class="styles['fg-instances__title-row']">
          <div>
            ...
            <p :class="styles['fg-instances__subtitle']">
              {{ t("editor.instances.subtitle") }}
            </p>
          </div>
          <div :class="styles['fg-instances__header-actions']">
            <el-button @click="openImportDialog">
              <AppIcon name="upload" class="el-icon--left" />{{
                t("editor.instances.import")
              }}
            </el-button>
            <el-button type="primary" @click="openCreateDialog">
              <AppIcon name="plus" class="el-icon--left" />{{
                t("editor.instances.create")
              }}
            </el-button>
          </div>
        </div>
        ...
        <div :class="styles['fg-instances__toolbar']">
          ...
          <div :class="styles['fg-instances__toolbar-right']">
            <el-input
              v-model="searchInput"
              :placeholder="t('editor.instances.searchPlaceholder')"
              clearable
              :class="styles['fg-instances__search']"
              @input="handleSearch"
              @clear="handleSearch('')"
            >
            ...
            >
              ...
              {{
                bulkMode
                  ? t("editor.instances.cancelBulk")
                  : t("editor.instances.bulkAction")
              }}
            </el-button>
            <el-button
      ...
      <!-- Loading -->
      <div
        v-if="store.loading && !store.hasSchemas"
        :class="styles['fg-instances__content']"
      >
        <div :class="styles['fg-instances__skeleton']">
          <div
            v-for="i in 6"
            :key="i"
            :class="styles['fg-instances__skeleton-card']"
          >
      ...
      >
        ...
        >
          <template #default>
            <el-button size="small" @click="store.fetchSchemas()">{{
              t("editor.instances.retry")
            }}</el-button>
          </template>
        </el-alert>
      </div>
      ...
      <div v-else :class="styles['fg-instances__content']">
        <div :class="styles['fg-instances__cards']">
          <div
            ...
            :key="item.id"
            :class="styles['fg-instances-card']"
          >
            ...
            <div
              :class="styles['fg-instances-card__preview']"
              @click="handleDesigner(item.id)"
            >
              <img
                v-if="item.thumbnail"
                :src="item.thumbnail"
                :alt="item.name"
                :class="styles['fg-instances-card__thumbnail']"
              />
              <div
                v-else
                :class="styles['fg-instances-card__thumbnail-placeholder']"
              >
            ...
            <div :class="styles['fg-instances-card__body']">
              ...
              <div :class="styles['fg-instances-card__meta']">
                ...
                <el-tag v-if="item.publishId" size="small" type="success">{{
                  t("editor.instances.published")
                }}</el-tag>
                <span
                  v-if="item.version"
                  :class="styles['fg-instances-card__version']"
                  >v{{ item.version }}</span
                >
                <!-- Component count -->
                <span
                  v-if="getJsonLength(item.json)"
                  :class="styles['fg-instances-card__count']"
                >
                  {{ getJsonLength(item.json) }}
                  {{ t("editor.instances.components") }}
                </span>
                <span :class="styles['fg-instances-card__date']">{{
            ...
            <div :class="styles['fg-instances-card__actions']">
              ...
              <el-tooltip
                :content="t('editor.instances.tooltipDesigner')"
                placement="top"
                :show-after="300"
              >
    ...
    <AppDialog
      ...
      :title="t('editor.instances.createDialogTitleNew')"
      width="520px"
    >
      <el-form label-width="96px" @submit.prevent="confirmCreate">
        <el-form-item :label="t('editor.instances.createFormName')">
          <el-input
            v-model="createName"
            :placeholder="t('editor.instances.createFormNamePlaceholder')"
            maxlength="100"
            show-word-limit
        ...
        <el-form-item :label="t('editor.instances.createFormLayoutMode')">
          <div :class="styles.createLayoutMode">
            <el-radio-group
              v-model="createLayoutMode"
              :class="styles.createLayoutRadios"
            ...
            <p v-if="selectedLayoutDesc" :class="styles.createLayoutDesc">
              {{ selectedLayoutDesc }}
            </p>
          </div>
        </el-form-item>
        ...
        <el-form-item v-else :label="t('editor.instances.createFormPreset')">
          <el-select v-model="createFreePreset" style="width: 100%">
            <el-option
              v-for="opt in freePresetOptions"
              :key="opt.value"
              :label="opt.label"
              :value="opt.value"
            />
        ...
        <el-form-item :label="t('editor.instances.createFormTypeTag')">
          ...
          <div :class="styles.createFieldHint">
            {{ t("editor.instances.createFormTypeHint") }}
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
        ...
        <el-button type="primary" @click="confirmCreate">{{
          t("editor.instances.createAndEdit")
        }}</el-button>
      </template>
    </AppDialog>
    ...
    <AppDialog
      ...
      :title="t('editor.instances.importDialogTitle')"
      width="440px"
    >
      <el-upload
        drag
      ...
      >
        ...
        <div class="el-upload__text">
          {{ t("editor.instances.importDragHint") }}
          <em>{{ t("editor.instances.importClickSelect") }}</em>
        </div>
        <template #tip>
          <div class="el-upload__tip">
            {{ t("editor.instances.importTip") }}
          </div>
        </template>
      </el-upload>
      <template #footer>
        ...
        <el-button
          ...
          @click="confirmImport"
          >{{ t("editor.instances.import") }}</el-button
        >
      </template>
    </AppDialog>
    ...
    <AppDialog
      ...
      :title="t('editor.instances.editDialogTitle')"
      width="440px"
    >
