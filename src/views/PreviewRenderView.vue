...
async function loadSchema(id: string) {
  ...
  try {
    ...
    schema.value = Array.isArray(result.json)
      ? result.json
      : ((result.json as any)?.widgets ?? []);
    schemaName.value = result.name;
  } catch (err: unknown) {
    error.value =
      err instanceof Error
        ? err.message
        : t("editor.previewView.schemaNotFound", { id });
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div :class="styles['fg-preview-render']">
    <div v-if="schemaName" :class="styles['fg-preview-render__banner']">
      ...
      <span :class="styles['fg-preview-render__banner-hint']">{{
        t("editor.previewView.draftHint")
      }}</span>
    </div>
    ...
    <div
      v-else-if="error"
      :class="[
        styles['fg-preview-render__state'],
        styles['fg-preview-render__state--error'],
      ]"
    >
      <p>{{ error }}</p>
    </div>
