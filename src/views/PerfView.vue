...
<template>
  ...
  >
    ...
    >
      ...
      >
        <label
          >{{ t("editor.perfView.widgetCount") }}
          <input
            v-model.number="widgetCount"
            type="number"
            min="20"
            max="500"
            step="10"
        ...
        <span v-if="mountMs !== null"
          >{{ t("editor.perfView.mountTime") }}
          <b>{{ mountMs.toFixed(0) }}ms</b></span
        >
        <span v-if="avgFps !== null"
          >{{ t("editor.perfView.avgFps") }} <b>{{ avgFps }}</b></span
        >
        <span v-if="minFps !== null"
          >{{ t("editor.perfView.minFps") }} <b>{{ minFps }}</b></span
        >
        <span v-if="maxFrameMs !== null"
          >{{ t("editor.perfView.maxFrame") }} <b>{{ maxFrameMs }}ms</b></span
        >
        <span v-if="jsHeapMb !== null"
          >{{ t("editor.perfView.jsHeap") }} <b>{{ jsHeapMb }}MB</b></span
        >
      </div>
    </div>
    ...
    <div style="padding: 16px; border-top: 1px solid #1f3a5f">
      <h3 style="margin: 0 0 8px">
        {{ t("editor.perfView.virtualTableTitle") }}
      </h3>
      <div
        style="
          display: flex;
      ...
      >
        <label
          >{{ t("editor.perfView.rowCount") }}
          <input
            v-model.number="virtualRowCount"
            type="number"
            min="100"
            max="100000"
            step="1000"
        ...
        <span v-if="virtualMountMs !== null"
          >{{ t("editor.perfView.mountRows") }}
          <b>{{ virtualMountMs }}ms</b>（{{ virtualRowCount }}
          {{ t("editor.perfView.rowsUnit") }}）</span
        >
      </div>
      <div
