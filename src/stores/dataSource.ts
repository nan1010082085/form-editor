...
export const useDataSourceStore = defineStore("dataSource", () => {
  ...
  async function fetch(dsId: string, force = false): Promise<unknown> {
    ...
    try {
      ...
      const mergedParams =
        Object.keys(filterParams.value).length > 0
  ...
  function connectWebSocket(dsId: string): void {
    ...
    ws.onclose = () => {
      ...
      const reconnect = () => {
        ...
        attempt++;
        setTimeout(
          () => connectWebSocket(dsId),
          delayMs * Math.min(attempt, 5),
        );
      };
      reconnect();
    };
  ...
  function subscribe(
    ...
    updateState(dsId, {
      subscriberCount: (states.value.get(dsId)?.subscriberCount ?? 0) + 1,
    });
  ...
  function updateState(dsId: string, patch: Partial<DataSourceState>): void {
    const prev = states.value.get(dsId) ?? {
      data: null,
      loading: false,
      error: null,
      lastUpdated: 0,
    ...
    if (mapping.childrenKey) {
      const mapTree = (items: Record<string, unknown>[]): DictItem[] =>
        items.map((item) => ({
          ...
          children:
            item[mapping.childrenKey!] &&
            Array.isArray(item[mapping.childrenKey!])
