...
import type {
  TemplateItem as WidgetTemplateItem,
  TemplateCategory,
} from "@/api/schemaApi";
...
async function loadTemplates() {
  ...
  try {
    ...
    ElMessage.error(
      err instanceof Error ? err.message : t("editor.templatePanel.loadFailed"),
    );
  } finally {
    loading.value = false;
  }
...
async function handleApply(template: WidgetTemplateItem): Promise<boolean> {
  try {
    await ElMessageBox.confirm(
      ...
      {
        confirmButtonText: t("editor.templatePanel.apply"),
        cancelButtonText: t("editor.common.cancel"),
      },
    );
  } catch {
...
<template>
  <div :class="styles.panel">
        ...
        >
          ...
          <div :class="styles.thumbnail">
            ...
            <span v-else :class="styles.placeholder">{{
              t("editor.templatePanel.noPreview")
            }}</span>
          </div>
          ...
          <div :class="styles.info">
            ...
            <div :class="styles.meta">
              <span :class="styles.category">{{
                ...
                t("editor.templatePanel.categoryOther")
              }}</span>
              <span v-if="template.isBuiltin" :class="styles.builtin">{{
                t("editor.templatePanel.builtin")
              }}</span>
            </div>
          </div>
          ...
          <div :class="styles.actions">
            ...
            <el-tooltip
              :content="t('editor.templatePanel.useTemplate')"
              placement="top"
              :show-after="300"
            >
              <el-button
    ...
    <AppDialog
      ...
      :title="
        previewTemplate
          ? t('editor.templatePanel.previewTitleWithName', {
              name: previewTemplate.name,
            })
          : t('editor.templatePanel.previewTitle')
      "
      width="720px"
