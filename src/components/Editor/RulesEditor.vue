...
function setRequired(index: number, val: boolean) {
  if (val) {
    ...
    emit(
      "update:rules",
      (props.rules ?? []).map((r, i) => (i === index ? rest : r)),
    );
  }
}
</script>
...
<template>
  <div :class="styles['rules-editor']">
    <div v-if="!rules?.length" :class="styles['rules-editor__empty']">
      {{ t("editor.rulesEditor.emptyHint") }}
    </div>

    <div
      v-for="(rule, idx) in rules ?? []"
      :key="idx"
      :class="styles['rules-editor__item']"
    >
      <div :class="styles['rules-editor__item-header']">
        <span :class="styles['rules-editor__item-title']">{{
          t("editor.rulesEditor.ruleTitle", { index: idx + 1 })
        }}</span>
      ...
      <div :class="styles['rules-editor__field']">
        <label :class="styles['rules-editor__label']">{{
          t("editor.rulesEditor.required")
        }}</label>
        <el-switch
          :model-value="rule.required === true"
      ...
      <div :class="styles['rules-editor__field']">
        ...
        <el-input
          ...
          placeholder="^[a-zA-Z]+$"
          @update:model-value="
            updateRule(idx, { pattern: $event || undefined })
          "
        />
      </div>
      ...
      <div :class="styles['rules-editor__field']">
        <label :class="styles['rules-editor__label']">{{
          t("editor.rulesEditor.errorMessage")
        }}</label>
        <el-input
          ...
          @update:model-value="
            updateRule(idx, { message: $event || undefined })
          "
        />
      </div>
      ...
      <div :class="styles['rules-editor__field']">
        <label :class="styles['rules-editor__label']">{{
          t("editor.rulesEditor.triggerMode")
        }}</label>
        <el-select
