...
const nameError = computed(() => {
  ...
  if (duplicates.length)
    return t("editor.variableConfig.nameDuplicate", { name: duplicates[0] });
  const invalid = localVariables.value.find(
    (v) => v.name && !/^[a-zA-Z_]\w*$/.test(v.name),
  );
  if (invalid)
    return t("editor.variableConfig.nameInvalid", { name: invalid.name });
  return "";
});
...
<template>
  ...
  >
    <div :class="styles.body">
      ...
      <div v-for="(v, i) in localVariables" :key="i" :class="styles.card">
        <div :class="styles.cardHeader">
          <span :class="styles.cardTitle">{{
            t("editor.variableConfig.variableTitle", { index: i + 1 })
          }}</span>
          <el-button type="danger" link size="small" @click="removeVariable(i)">
            <AppIcon name="delete" />
          </el-button>
        </div>
        ...
        <div :class="styles.row">
          ...
          <label :class="styles.label" style="margin-left: 8px">{{
            t("editor.variableConfig.defaultValue")
          }}</label>
          <el-switch v-if="v.type === 'boolean'" v-model="v.defaultValue" />
          <el-input-number
        ...
        <div :class="styles.row">
          <label :class="styles.label">{{
            t("editor.variableConfig.description")
          }}</label>
          <el-input
      ...
      <!-- 添加变量 -->
      <el-button type="primary" plain style="width: 100%" @click="addVariable">
        <AppIcon name="plus" />
    ...
    <template #footer>
      ...
      <el-button type="primary" :disabled="!!nameError" @click="handleSave">{{
