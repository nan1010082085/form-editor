...
describe("evaluateCondition", () => {
  ...
  it("returns false for non-matching expression", () => {
    expect(
      evaluateCondition('status === "active"', { status: "inactive" }),
    ).toBe(false);
  });
  ...
  it("evaluates expression with exposed context", () => {
    ...
    expect(
      evaluateCondition(
        "exposed.table1.selectedRows.length > 0",
        context,
        exposed,
      ),
    ).toBe(true);
  ...
  it("evaluates expression with exposed and variables together", () => {
    ...
    expect(
      evaluateCondition(
        'mode === "edit" && variables.debug && !exposed.form1.loading',
        context,
        exposed,
      ),
    ).toBe(true);
...
describe("executeEventAction", () => {
  ...
  function createMockContext(
    overrides: Partial<EventExecutionContext> = {},
  ...
  it("set-value updates target widget defaultValue", () => {
    vi.mocked(ctx.findWidget).mockReturnValue({
      id: "w1",
      defaultValue: "old",
    } as any);
    executeEventAction({ type: "set-value", target: "w1", value: "new" }, ctx);
    expect(ctx.updateWidget).toHaveBeenCalledWith("w1", {
      defaultValue: "new",
    });
  });
  ...
  it("switch-tab updates tabs widget activeKey", () => {
    ...
    executeEventAction(
      { type: "switch-tab", target: "tabs1", value: "tab2" },
      ctx,
    );
    expect(ctx.updateWidget).toHaveBeenCalledWith("tabs1", {
      props: { activeKey: "tab2" },
    });
  });
  ...
  describe("set-variable", () => {
    it("calls ctx.setVariable with variable name and value", () => {
      ...
      executeEventAction(
        { type: "set-variable", variable: "count", value: 42 },
        ctx,
      );
  ...
  describe("post-message", () => {
    ...
    it("sends postMessage to parent window", () => {
      ...
      executeEventAction(
        { type: "post-message", message: { type: "save", data: "test" } },
        ctx,
      );
    ...
    it("resolves formData.xxx references in message", () => {
      ...
      window.parent.postMessage = postMessageSpy;
      ctx = createMockContext({
        getFormData: vi.fn().mockReturnValue({ userName: "Alice" }),
      });
      executeEventAction(
        { type: "post-message", message: { name: "formData.userName" } },
        ctx,
      );
      expect(postMessageSpy).toHaveBeenCalledWith(
        { name: "Alice" },
  ...
  describe("copy", () => {
    ...
    it("resolves formData.xxx references in text", async () => {
      ...
      Object.assign(navigator, { clipboard: { writeText: writeTextSpy } });
      ctx = createMockContext({
        getFormData: vi.fn().mockReturnValue({ email: "test@example.com" }),
      });
  ...
  describe("api", () => {
    it("calls apiClient.requestUrl and emits api-success", async () => {
      ...
      await executeEventAction(
        { type: "api", apiUrl: "/api/users", apiMethod: "get" },
        ctx,
      );
      ...
      expect(ctx.emit).toHaveBeenCalledWith("api-success", {
        url: "/api/users",
        response: { ok: true },
      });
    });
    ...
    it('sends formData as raw body when apiParams is "formData"', async () => {
      ...
      mockRequestUrl.mockResolvedValue({});
      await executeEventAction(
        { type: "api", apiUrl: "/api/save", apiParams: "formData" },
        ctx,
      );
      expect(mockRequestUrl).toHaveBeenCalledWith("post", "/api/save", {
        name: "test",
      });
    });
    ...
    it("emits api-error on request failure", async () => {
      ...
      expect(ctx.emit).toHaveBeenCalledWith("api-error", {
        url: "/api/fail",
    ...
    it("sends custom params", async () => {
      mockRequestUrl.mockResolvedValue({});
      await executeEventAction(
        { type: "api", apiUrl: "/api/query", apiParams: { id: 123 } },
        ctx,
      );
      expect(mockRequestUrl).toHaveBeenCalledWith("post", "/api/query", {
        id: 123,
      });
    });
  ...
  describe("submitSubmission", () => {
    it("validates, posts { data }, emits submission-created", async () => {
      ...
      expect(mockCreateSubmission).toHaveBeenCalledWith("schema-leave", {
        leaveType: "annual",
      });
    ...
    it("skips when validation fails", async () => {
      ...
      await executeEventAction(
        { type: "submitSubmission", schemaId: "x" },
        ctx,
      );
  ...
  describe("startFlow", () => {
    ...
    it("uses empty variables when not provided", async () => {
      ...
      await executeEventAction(
        { type: "startFlow", definitionId: "def-456" },
        ctx,
      );
      expect(mockRequestUrl).toHaveBeenCalledWith("post", "/flow-instances", {
    ...
    it("emits flow-error on request failure", async () => {
      ...
      await executeEventAction(
        { type: "startFlow", definitionId: "def-err" },
        ctx,
      );
  ...
  describe("endFlow", () => {
    it("calls POST /flow-instances/:id/terminate", async () => {
      ...
      expect(mockRequestUrl).toHaveBeenCalledWith(
        "post",
        "/flow-instances/inst-1/terminate",
        { reason: "用户取消" },
      );
    ...
    it("omits reason body when not provided", async () => {
      ...
      expect(mockRequestUrl).toHaveBeenCalledWith(
        "post",
        "/flow-instances/inst-2/terminate",
        undefined,
      );
    });
    ...
    it("emits flow-error on request failure", async () => {
      ...
      await executeEventAction(
        { type: "endFlow", instanceId: "inst-err" },
        ctx,
      );
      expect(ctx.emit).toHaveBeenCalledWith("flow-error", {
        action: "endFlow",
  ...
  describe("exportData (E-21)", () => {
    ...
    it("downloads file from resolved apiUrl", async () => {
      ...
      expect(ctx.emit).toHaveBeenCalledWith(
        "export-success",
        expect.objectContaining({ url: "/submissions/record/abc123/export" }),
      );
    });
...
describe("triggerWidgetEvent", () => {
  ...
  it("executes action chain in order", async () => {
    ...
    vi.mocked(ctx.submitForm).mockImplementation(() => {
      callOrder.push("submit");
    });
    vi.mocked(ctx.resetForm).mockImplementation(() => {
