...
function makeWidget(
  overrides: Partial<Widget> & { id: string; type: Widget["type"] },
): Widget {
  return {
...
describe("diffSchema", () => {
  ...
  it("detects added widgets", () => {
    ...
    expect(result.added.map((d) => d.id)).toEqual(
  ...
  it("detects modified widgets when label changes", () => {
    ...
    const labelChange = result.modified[0].changes!.find((c) =>
      c.field.includes("label"),
    );
  ...
  it("detects modified widgets when props change", () => {
    ...
    const result = diffSchema(oldWidgets, newWidgets);
    expect(result.modified).toHaveLength(1);
    const propChange = result.modified[0].changes!.find((c) =>
      c.field.includes("placeholder"),
    );
  ...
  it("detects moved widgets (parent change)", () => {
    const oldWidgets: Widget[] = [
      makeWidget({
        ...
        type: "card",
        name: "Card-1",
        children: [makeWidget({ id: "child-a", type: "input", label: "A" })],
      }),
      makeWidget({
        id: "parent2",
        type: "card",
  ...
  it("detects widget moved from root to container", () => {
    ...
    const newWidgets: Widget[] = [
      makeWidget({
        ...
        type: "card",
        name: "Card",
        children: [makeWidget({ id: "a", type: "input", label: "A" })],
      }),
    ];
    const result = diffSchema(oldWidgets, newWidgets);
    expect(result.moved).toHaveLength(1);
  ...
  it("detects diffs in nested children", () => {
    const oldWidgets: Widget[] = [
      makeWidget({
        ...
        type: "form",
        name: "Form",
        children: [makeWidget({ id: "a", type: "input", label: "Name" })],
      }),
    ];
    const newWidgets: Widget[] = [
      makeWidget({
  ...
  it("detects multiple diff types simultaneously", () => {
    const oldWidgets: Widget[] = [
      ...
      makeWidget({
        ...
        type: "card",
        name: "A",
        children: [makeWidget({ id: "mover", type: "input", label: "M" })],
      }),
