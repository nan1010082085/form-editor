...
describe("碰撞检测 — 深层嵌套", () => {
  ...
  describe("collectAllContainers", () => {
    ...
    it("支持 3 层以上嵌套", () => {
      ...
      const level2 = makeContainer("level2", "form", 10, 10, 580, 180, [
        level3,
      ]);
  ...
  describe("detectNestedContainerCollision", () => {
    it("检测到最深层容器（面积优先）", () => {
      ...
      const inner = makeContainer(
        ...
        580,
        380,
      ) as Widget & { _canvasX: number; _canvasY: number; _depth: number };
      inner._canvasX = 10;
      inner._canvasY = 10;
      inner._depth = 1;

      const outer = makeContainer("outer", "form", 0, 0, 600, 400) as Widget & {
        _canvasX: number;
        _canvasY: number;
        _depth: number;
      };
    ...
    it("深度优先：同面积时选最深容器", () => {
      ...
      const deep = makeContainer("deep", "card", 0, 0, 400, 300) as Widget & {
        _canvasX: number;
        _canvasY: number;
        _depth: number;
      };
