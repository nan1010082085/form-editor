...
function mountWithEl(
  ...
  return mount(component as Parameters<typeof mount>[0], {
    global: {
      ...
      stubs: { "el-form-item": { template: "<div><slot /></div>" } },
    },
    ...options,
  });
}
      ...
      linkages: [
        {
          type: "visible",
          watchFields: ["trigger"],
          condition: 'values.trigger === "a"',
        },
      ],
    });
  }
  return [{ type: "card", name: "FgCard", children: items }];
}
...
describe("Undo/redo performance", () => {
  it("pushHistory + undo/redo on 100-widget schema < 50ms each", () => {
    ...
    for (let i = 0; i < 10; i++) {
      store.pushHistory(
        widgets.map((w) => ({ ...w, label: `${w.label} v${i}` })),
      );
    }
