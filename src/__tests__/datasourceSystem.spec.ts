...
describe("数据源系统", () => {
  ...
  describe("响应数据标准化", () => {
    ...
    it("提取 total 使用 totalPath", () => {
      ...
      const { data, total } = normalizeListResponse(res, {
        totalPath: "pageInfo.totalCount",
      });
  ...
  describe("缓存系统", () => {
    ...
    it("不同参数独立缓存", () => {
      ...
      expect(getCachedOptions("/api/test", { type: "a" })).toEqual([
        { label: "A", value: "a" },
      ]);
      expect(getCachedOptions("/api/test", { type: "b" })).toEqual([
        { label: "B", value: "b" },
      ]);
    });
