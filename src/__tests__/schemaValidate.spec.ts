...
describe("validateSchema", () => {
  ...
  it("warns about container with no children at all", () => {
    const schema: PartialWidget[] = [{ type: "card", label: "No Kids Card" }];
    const result = validateSchema(schema);
    const emptyErrors = result.errors.filter(
      (e) => e.type === "empty-container",
    );
    ...
    const schema: PartialWidget[] = [
      {
        ...
        field: "users",
        label: "Users",
        children: [{ type: "input", field: "desc", label: "Description" }],
      },
    ];
    const result = validateSchema(schema);
    const nestingErrors = result.errors.filter(
      (e) => e.type === "nesting-violation",
    );
    ...
    const schema: PartialWidget[] = [
      {
        ...
        buttons: [{ text: "Submit" }],
        children: [{ type: "user-selector", field: "user", label: "User" }],
      },
    ];
    const result = validateSchema(schema);
    const nestingErrors = result.errors.filter(
        ...
        children: [
          {
            ...
            field: "users",
            label: "Users",
            children: [{ type: "input", field: "desc", label: "Desc" }],
          },
        ],
      },
    ];
    ...
    const schema: PartialWidget[] = [
      {
        ...
        field: "tbl",
        label: "Table",
        children: [{ type: "input", field: "cell", label: "Cell" }],
      },
    ];
    const result = validateSchema(schema);
    const nestingErrors = result.errors.filter(
    ...
    const schema: PartialWidget[] = [
      {
        ...
        field: "detail",
        label: "Detail",
        children: [{ type: "upload", field: "file", label: "File" }],
      },
    ];
    const result = validateSchema(schema);
    const nestingErrors = result.errors.filter(
