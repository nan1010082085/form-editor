      ...
      setup(_, { slots }) {
        return () =>
          h(
            "div",
            { class: className },
            isContainer && slots.default ? slots.default() : [],
          );
      },
    });
  }
...
function mountSchemaRender(
  ...
  const linkage = useLinkage(
    ...
    computed(
      () => formData as import("@/components/WidgetRenderer/types").FormData,
    ),
  );
...
describe("SchemaRender", () => {
  ...
  describe("Basic rendering", () => {
    ...
    it("renders wrapper div but no component for unknown widget type", () => {
      ...
      expect(wrapper.find('div[style*="position: absolute"]').exists()).toBe(
        true,
      );
    });
  });
  ...
  describe("Container rendering", () => {
    ...
    for (const containerType of containerTypes) {
      it(`renders ${containerType} container with children`, () => {
        ...
        const container = makeContainerWidget(containerType, [child], {
          id: "container1",
        });
      ...
      it(`renders ${containerType} without children gracefully`, () => {
        const container = makeContainerWidget(containerType, [], {
          id: "container1",
        });
    ...
    it("renders column containers as self-rendering (children delegated to component)", () => {
      ...
      for (const colType of colTypes) {
        const child = makeWidget({ type: "input", id: "child1" });
        const container = makeContainerWidget(colType, [child], {
          id: "container1",
        });
        const wrapper = mountSchemaRender([container]);
    ...
    it("renders dialog as EnhancedDialog in preview mode (default)", () => {
      const child = makeWidget({ type: "input", id: "child1" });
      const container = makeContainerWidget("dialog", [child], {
        id: "dialog1",
      });
  ...
  describe("Mixed layout", () => {
    it("renders a form with card children containing inputs", () => {
      ...
      const card = makeContainerWidget("card", inputs, {
        id: "card1",
        label: "Info",
      });
    ...
    it("renders multiple root-level containers", () => {
      const form = makeContainerWidget(
        "form",
        [makeWidget({ type: "input", id: "i1" })],
        { id: "form1" },
      );
      const card = makeContainerWidget(
        "card",
  ...
  describe("Non-container rendering", () => {
    const nonContainerTypes: SchemaType[] = [
      "input",
      "select",
      "textarea",
      "date",
