...
describe("变量系统", () => {
  ...
  describe("set-variable 动作", () => {
    ...
    it("设置字符串变量", () => {
      executeEventAction(
        { type: "set-variable", variable: "name", value: "updated" },
        ctx,
      );
    ...
    it("设置布尔变量", () => {
      executeEventAction(
        { type: "set-variable", variable: "active", value: true },
        ctx,
      );
    ...
    it("设置对象变量", () => {
      executeEventAction(
        { type: "set-variable", variable: "config", value: { theme: "dark" } },
        ctx,
      );
    ...
    it("设置数组变量", () => {
      executeEventAction(
        { type: "set-variable", variable: "items", value: [1, 2, 3] },
        ctx,
      );
    ...
    it("变量值变化后可通过 getVariable 获取", () => {
      executeEventAction(
        { type: "set-variable", variable: "count", value: 100 },
        ctx,
      );
    ...
    it("连续多次设置同一变量", () => {
      executeEventAction(
        { type: "set-variable", variable: "count", value: 1 },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "count", value: 2 },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "count", value: 3 },
        ctx,
      );
  ...
  describe("变量在条件表达式中", () => {
    it("variables.xxx 引用", () => {
      ...
      expect(evaluateCondition("variables.threshold < 50", context)).toBe(
        false,
      );
    });
    ...
    it("变量支持复杂表达式", () => {
      ...
      expect(
        evaluateCondition("variables.min < 50 && variables.max > 50", context),
      ).toBe(true);
    });
          ...
          linkages: [
            {
              ...
              watchFields: ["type"],
              condition:
                'values.type === "advanced" && variables.enableAdvanced === true',
            },
          ],
        },
      ];
  ...
  describe("组件暴露值", () => {
    it("exposed 上下文在条件中生效", () => {
      ...
      expect(
        evaluateCondition("exposed.form1.loading === false", {}, exposed),
      ).toBe(true);
    });
    ...
    it("多个组件暴露值独立引用", () => {
      ...
      expect(
        evaluateCondition(
          "exposed.table2.selectedRows.length > 0",
          {},
          exposed,
        ),
      ).toBe(false);
        ...
        events: [
          {
            ...
            actions: [
              { type: "submit" },
              {
                ...
                value: (variables.actionCount as number) + 1,
              },
            ],
          },
        ],
          ...
          linkages: [
            {
              ...
              watchFields: ["status"],
              condition:
                'values.status === "draft" || variables.readonly || exposed.form1.loading',
            },
          ],
        },
      ];
