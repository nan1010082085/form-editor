...
function createTestHarness(options: {
  ...
  return defineComponent({
    ...
    setup() {
      ...
      provide(
        FORM_GRID_READONLY_KEY,
        computed(() => options.globalReadonly ?? false),
      );
      provide(
        FORM_GRID_READONLY_FIELDS_KEY,
        computed(() => options.readonlyFields),
      );
      provide(
        FORM_GRID_EDITABLE_FIELDS_KEY,
        computed(() => options.editableFields),
      );
...
describe("useWidgetRenderState", () => {
  ...
  it("returns disabled=true when widget props.disabled is true", () => {
    const wrapper = mount(
      createTestHarness({ widgetField: "name", widgetDisabled: true }),
    );
