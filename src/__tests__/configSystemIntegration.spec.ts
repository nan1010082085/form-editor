...
describe("三大配置系统 + 变量系统 交互集成", () => {
  ...
  describe("事件 ↔ 变量", () => {
    ...
    it("set-variable 覆盖已有变量", () => {
      ...
      executeEventAction(
        { type: "set-variable", variable: "status", value: "running" },
        ctx,
      );
    ...
    it("set-variable 创建新变量", () => {
      ...
      executeEventAction(
        { type: "set-variable", variable: "newVar", value: "hello" },
        ctx,
      );
    ...
    it("set-variable 支持不同值类型", () => {
      ...
      executeEventAction(
        { type: "set-variable", variable: "str", value: "text" },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "num", value: 123 },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "bool", value: true },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "arr", value: [1, 2, 3] },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "obj", value: { a: 1 } },
        ctx,
      );
      executeEventAction(
        { type: "set-variable", variable: "nil", value: null },
        ctx,
      );
    ...
    it("set-variable 无 setVariable 回调时静默跳过", () => {
      ...
      executeEventAction(
        { type: "set-variable", variable: "x", value: 99 },
        ctx,
      );
    ...
    it("set-variable 无 variable 名称时跳过", () => {
      ...
      executeEventAction(
        { type: "set-variable", variable: "", value: "test" },
        ctx,
      );
        ...
        events: [
          {
            trigger: "click",
            condition:
              'values.status === "active" && variables.role === "admin"',
            actions: [{ type: "submit" }],
          },
        ],
      });
    ...
    it("trigger-event 无 target 时跳过", () => {
      ...
      executeEventAction(
        { type: "trigger-event", target: "", event: "refresh" },
        ctx,
      );
    ...
    it("trigger-event 无 event 时跳过", () => {
      ...
      executeEventAction(
        { type: "trigger-event", target: "table1", event: "" },
        ctx,
      );
      expect(triggerEvent).not.toHaveBeenCalled();
    });
          ...
          linkages: [
            {
              ...
              watchFields: ["status"],
              condition:
                'values.status !== "ready" || exposed.counter.value < variables.minCount',
            },
          ],
        },
      ];
          ...
          linkages: [
            {
              type: "visible",
              watchFields: ["mode"],
              condition: 'values.mode !== "hidden"',
            },
            {
              type: "required",
              watchFields: ["mode"],
              condition: 'values.mode === "full"',
            },
            {
              type: "disabled",
              watchFields: ["mode"],
  ...
  describe("数据源 ↔ 联动", () => {
    ...
    it("多个 options 联动条件互斥时只生效一个", () => {
      ...
      formData.parent = "A";
      expect(stateMap.value.get("sub")!.options).toEqual([
        { label: "A1", value: "a1" },
      ]);
      ...
      const schema: PartialWidget[] = [
        {
          ...
          events: [
            {
              trigger: "click",
              actions: [{ type: "open-dialog", target: "editDlg" }],
            },
          ],
        },
        {
  ...
  describe("边界场景", () => {
    ...
    it("switch-tab 动作更新 tabs activeKey", () => {
      ...
      executeEventAction(
        { type: "switch-tab", target: "w1", value: "tab2" },
        ctx,
      );
