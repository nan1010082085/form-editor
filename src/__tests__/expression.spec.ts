...
describe("expression engine", () => {
  ...
  describe("basic evaluation", () => {
    ...
    it("evaluates comparison operators", () => {
      expect(evaluateExpression<boolean>("${age} > 18", ctx({ age: 20 }))).toBe(
        true,
      );
      expect(
        evaluateExpression<boolean>("${age} >= 18", ctx({ age: 18 })),
      ).toBe(true);
      expect(evaluateExpression<boolean>("${age} < 18", ctx({ age: 15 }))).toBe(
        true,
      );
      expect(
        evaluateExpression<boolean>("${age} <= 18", ctx({ age: 18 })),
      ...
      expect(
        evaluateExpression<boolean>("${age} !== 18", ctx({ age: 20 })),
      ).toBe(true);
    });
    ...
    it("evaluates logical operators", () => {
      ...
      expect(
        evaluateExpression<boolean>("!${disabled}", ctx({ disabled: false })),
      ).toBe(true);
    });
  ...
  describe("security blocking", () => {
    it("blocks window access", () => {
      expect(() => evaluateExpression("window.location", ctx())).toThrow(
        "window",
      );
    });

    it("blocks document access", () => {
      expect(() => evaluateExpression("document.title", ctx())).toThrow(
        "document",
      );
    });
    ...
    it("blocks require()", () => {
      expect(() => evaluateExpression('require("fs")', ctx())).toThrow(
        "require",
      );
    });

    it("blocks import()", () => {
      expect(() => evaluateExpression('import("./evil")', ctx())).toThrow(
        "import",
      );
    });
    ...
    it("blocks globalThis access", () => {
      expect(() => evaluateExpression("globalThis.fetch", ctx())).toThrow(
        "globalThis",
      );
    });
  ...
  describe("compile cache", () => {
    ...
    it("evicts oldest entry when cache reaches 1000", () => {
      ...
      for (let i = 0; i < 1000; i++) {
        evaluateExpression(`\${a${i}} > 0`, {
          formData: { [`a${i}`]: 1 } as never,
        });
      }
      ...
      const result = evaluateExpression("${a0} > 0", {
        formData: { a0: 5 } as never,
      });
      expect(result).toBe(true);
    });
  });
