 ...
 * - toggle: delegates to updateTenant
 */
import { describe, it, expect, beforeEach, vi } from "vitest";
import { createPinia, setActivePinia } from "pinia";
import { useTenantStore } from "@/stores/tenant";
import type { TenantItem } from "@/types/tenant";
...
describe("useTenantStore", () => {
  ...
  it("fetches tenants and updates state", async () => {
    const items = [
      makeTenant(),
      makeTenant({ id: "t-2", name: "Tenant 2", code: "t2" }),
    ];
  ...
  it("updates tenant and patches local list", async () => {
    ...
    expect(mockPut).toHaveBeenCalledWith("/tenants/tenant-001", {
      name: "Updated",
    });
