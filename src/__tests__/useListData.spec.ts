...
describe("useListData", () => {
  ...
  describe("initial state", () => {
    it("returns empty tableData, total=0, loading=false", () => {
      const { tableData, total, loading, error, currentPage, pageSize } =
  ...
  describe("handleSearch", () => {
    it("resets page to 1 by default", async () => {
      ...
      const lastCall =
        mockRequestUrl.mock.calls[mockRequestUrl.mock.calls.length - 1];
  ...
  describe("handleSizeChange", () => {
    it("resets page to 1 and updates pageSize", async () => {
      ...
      const { handlePageChange, handleSizeChange, currentPage, pageSize } =
        useListData({
          listApi: defaultListApi,
          autoLoad: false,
        });
  ...
  describe("selectedRows", () => {
    it("updates selectedRows on selection change", () => {
      const { selectedRows, handleSelectionChange, clearSelection } =
        useListData({
          listApi: defaultListApi,
          autoLoad: false,
        });
