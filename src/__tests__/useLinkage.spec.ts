...
describe("useLinkage", () => {
          ...
          linkages: [
            {
              type: "options",
              watchFields: ["source"],
              condition: 'values.source === "x"',
              thenOptions: [{ label: "New", value: "new" }],
              elseValue: "",
            },
          ],
        },
  ...
  describe("reset-fields linkage", () => {
    it("stores resetFields when condition is met", () => {
      ...
      const { stateMap } = useLinkage(schema, formData);
      expect(stateMap.value.get("trigger")!.resetFields).toEqual([
        "field1",
        "field2",
      ]);
    });
          ...
          linkages: [
            {
              ...
              watchFields: ["type"],
              condition:
                'values.type === "detail" && variables.debug && exposed.table1.selectedRows.length > 0',
            },
          ],
        },
      ];
