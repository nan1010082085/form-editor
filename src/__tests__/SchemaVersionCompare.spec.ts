...
vi.mock("@/stores/editor", () => ({
  useEditorStore: () => ({ markClean: vi.fn(), resetHistory: vi.fn() }),
}));
...
async function setupWithVersions(
...
async function triggerCompareFlow(wrapper: any, store: any) {
  ...
  const compareBtn = buttons.find((b: any) =>
    b.text().includes("editor.versionCompare.compareSelected"),
  );
  if (compareBtn) {
...
describe("SchemaVersionCompare", () => {
  ...
  it("store: executeCompare with identical schemas", async () => {
    ...
    mockFetchVersion
      .mockResolvedValueOnce(
        makeSchemaDetail({ version: "v1", json: makeWidgets() }),
      )
      .mockResolvedValueOnce(
        makeSchemaDetail({ version: "v2", json: makeWidgets() }),
      );
        ...
        makeSchemaDetail({
          ...
          json: [
            ...
            },
            {
              ...
              position: { x: 0, y: 150, w: 400, h: 200 },
            },
            {
              id: "parent-b",
              type: "card",
              name: "B",
              position: { x: 0, y: 360, w: 400, h: 200 },
            },
          ],
        }),
      )
  ...
  it("shows pagination when total exceeds page size", async () => {
    ...
    const versions = Array.from({ length: 20 }, (_, i) =>
      makeVersionEntry({
        version: `202601011200${String(i).padStart(2, "0")}`,
      }),
    );
