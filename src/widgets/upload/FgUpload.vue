...
const accept = computed(
  () => (widgetData.value.props?.accept as string) || undefined,
);
const multiple = computed(() => Boolean(widgetData.value.props?.multiple));
const limit = computed(
  () => (widgetData.value.props?.limit as number) || undefined,
);
const maxSizeMb = computed(
  () => (widgetData.value.props?.maxSize as number) || 10,
);
const buttonText = computed(
  () =>
    (widgetData.value.props?.buttonText as string) ||
    t("editor.upload.clickToUpload"),
);
const listType = computed(
  () =>
    ((widgetData.value.props?.listType as string) || "text") as
      | "text"
      | "picture"
      | "picture-card",
);
...
function beforeUpload(rawFile: File) {
  ...
  if (rawFile.size > maxBytes) {
    ElMessage.warning(
      t("editor.upload.fileSizeExceeded", { size: maxSizeMb.value }),
    );
    return false;
  }
  return true;
