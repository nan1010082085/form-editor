...
const title = computed(
  () =>
    ...
    t("editor.notification.defaultTitle"),
);
const pageSize = computed(() => Number(widgetData.value.props?.pageSize ?? 5));
const source = computed(
  () => (widgetData.value.props?.source as string) || "notices",
);
  ...
  <div :class="styles.wrapper">
    ...
    <ul v-else-if="items.length" :class="styles.list">
      <li v-for="item in items" :key="item.id" :class="styles.item">
        ...
        <div v-if="item.content" :class="styles.itemContent">
          {{ item.content }}
        </div>
        <div :class="styles.itemTime">
          {{ formatTime((item as NoticeItem).publishAt ?? item.createdAt) }}
        </div>
      </li>
    </ul>
