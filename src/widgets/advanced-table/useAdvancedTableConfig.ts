...
export function useAdvancedTableConfig(
  ...
  const tableHeight = computed(
    () => (widgetData.value.props?.height as number) ?? 350,
  );
  const globalSortable = computed(
    () => (widgetData.value.props?.sortable as boolean) ?? false,
  );
  const virtualEnabled = computed(
    () => (widgetData.value.props?.virtual as boolean) ?? false,
  );
  ...
  const selectionConfig = computed<AdvSelectionConfig>(
    () =>
      (widgetData.value.props?.selection as AdvSelectionConfig) ?? {
        enabled: false,
      },
  );
  ...
  const searchBarConfig = computed<SearchBarConfig>(
    () =>
      (widgetData.value.props?.searchBar as SearchBarConfig) ?? {
        enabled: false,
        fields: [],
      },
  );
