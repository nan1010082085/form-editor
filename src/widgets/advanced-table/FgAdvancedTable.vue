...
function defaultFilterMethod(prop: string) {
  ...
  return (value: unknown, row: Record<string, unknown>) =>
    getRowCellValue(row, prop) === value;
}
...
const useVirtual = computed(
  () => virtualEnabled.value && tableData.value.length > VIRTUAL_THRESHOLD,
);
...
<template>
  <div :class="styles.container">
    ...
    <!-- Batch actions bar -->
    <div
      v-if="selectionConfig.enabled && selectedRows.length > 0"
      :class="styles.batchBar"
    >
      <span :class="styles.batchInfo">{{
        t("editor.advancedTable.batchSelected", { count: selectedRows.length })
      }}</span>
      <el-button size="small" text @click="clearSelection">{{
        t("editor.advancedTable.cancelSelection")
      }}</el-button>
    </div>
    ...
    >
      ...
      <FgAdvancedTableVirtual
        ...
        :data="tableData"
        :height="tableHeight"
        @row-click="(row: Record<string, unknown>) => onRowClick(row)"
        @row-button-click="
          (btn: ActionButton, row: Record<string, unknown>, i: number) =>
            handleRowButtonClick(btn, row, i)
        "
      />
      <!-- 常规路径：el-table（全特性） -->
      <el-table
        ...
        >
          <template #default="{ row, $index }">
            ...
            <template v-else-if="col.render === 'tooltip'">
              <el-tooltip
                :content="getTooltipContent(col, row)"
                placement="top"
              >
            ...
            <template v-else-if="col.render === 'link'">
              <el-link
                type="primary"
                @click.stop="handleLinkClick(col, row, $index)"
              >
                {{ cellValue(row, col.prop) }}
              </el-link>
            </template>
            ...
            <template v-else-if="col.render === 'tag'">
              ...
              >
                {{
                  getOptionLabel(
                    effectiveColumn(col).options as any,
                    cellValue(row, col.prop),
                  )
                }}
              </el-tag>
            ...
            <template v-else-if="col.render === 'flowStatus'">
              <el-tag :type="getFlowStatusTagType(row) as any" size="small">
            ...
            <template v-else-if="col.render === 'expiryAlert'">
              <el-tag :type="getExpiryTagType(row) as any" size="small">
                {{ getExpiryLabel(row) }}
              </el-tag>
            </template>
