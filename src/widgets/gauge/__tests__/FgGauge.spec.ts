...
describe("FgGauge", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_gauge")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_gauge")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("gauge", "test_gauge")!;
      store.addWidget(widget);
      store.updateWidget("test_gauge", {
        props: { valueField: "score", min: 0, max: 200, unit: "%" },
      });
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_gauge")!.events).toHaveLength(1);
      expect(store.findWidget("test_gauge")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
