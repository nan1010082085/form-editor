...
describe("FgTransfer", () => {
  ...
  describe("store CRUD", () => {
    ...
    it("更新标题后 store 中同步", () => {
      const widget = createWidget("transfer", "test_transfer")!;
      store.addWidget(widget);
      store.updateWidget("test_transfer", {
        props: { ...widget.props, leftTitle: "可选", rightTitle: "已选" },
      });
  ...
  describe("标题属性", () => {
    ...
    it("自定义标题", () => {
      const wrapper = mountWidget({
        props: { leftTitle: "可选项目", rightTitle: "已选项目" },
      });
  ...
  describe("事件", () => {
    it("支持 change 事件配置", () => {
      ...
      widget.events = [
        {
          trigger: "change",
          actions: [{ type: "set-value", target: "form", value: {} }],
        },
      ];
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.events).toHaveLength(1);
      expect(store.findWidget("test_widget")!.events![0].trigger).toBe(
        "change",
      );
    });
    ...
    it("支持 left-check-change 事件", () => {
      ...
      widget.events = [
        {
          trigger: "left-check-change",
          actions: [{ type: "emit", target: "parent" }],
        },
      ];
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.events![0].trigger).toBe(
