...
import { widgetDataKey, type FormFieldValue } from "../base/types";
...
const selectedKeys = computed({
  ...
  set: (keys: Array<string | number>) => {
    widgetData.value.defaultValue = keys as unknown as FormFieldValue;
  },
});
...
const titles = computed(() => [
  (widgetData.value.props?.leftTitle as string) ||
    t("editor.transfer.leftTitle"),
  (widgetData.value.props?.rightTitle as string) ||
    t("editor.transfer.rightTitle"),
]);
