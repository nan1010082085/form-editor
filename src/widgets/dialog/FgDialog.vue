...
const contentMode = computed(
  () => (widgetData.value.props?.contentMode as string) ?? "edit",
);
const microappName = computed(
  () => (widgetData.value.props?.microappName as string) ?? "",
);
const microappEntry = computed(
  () => (widgetData.value.props?.microappEntry as string) ?? "",
);
const microappRoute = computed(
  () => (widgetData.value.props?.microappRoute as string) ?? "",
);
const sandbox = computed(
  () => widgetData.value.props?.microappSandbox !== false,
);
const styleIsolation = computed(
  () =>
    (widgetData.value.props?.microappStyleIsolation as string) ??
    "experimental",
);
const timeout = computed(
  () => (widgetData.value.props?.microappTimeout as number) ?? 10000,
);
const fallbackText = computed(
  () =>
    (widgetData.value.props?.microappFallback as string) ??
    t("editor.microAppContainer.fallbackError"),
);
const routeBase = computed(
  () => (widgetData.value.props?.microappRouteBase as string) ?? "",
);
...
async function loadMicroAppDynamic() {
  ...
  timeoutTimer = setTimeout(() => {
    if (microappLoading.value) {
      microappError.value = t("editor.microAppContainer.loadTimeout", {
        ms: timeout.value,
      });
  ...
  try {
    ...
    microAppInstance = loadMicroApp(
      {
        name: microappName.value,
        entry: microappEntry.value,
...
<template>
  ...
  <div v-if="editable" :class="styles.dialogShell">
    <div :class="styles.dialogHeader">
      <span :class="styles.dialogTitle">{{
        (widgetData.props?.title as string) || t("editor.dialog.defaultTitle")
      }}</span>
    </div>
  </div>
  ...
  <template v-else>
    ...
    >
      <!-- 微应用模式 -->
      <div
        v-if="contentMode === 'microapp'"
        style="height: 100%; min-height: 200px"
      >
        <div
          v-if="!microappName || !microappEntry"
          style="
            display: flex;
            align-items: center;
      ...
      <template v-if="widgetData.props?.showFooter !== false" #footer>
        <div :class="styles.footer">
          <el-button @click="handleCancel">{{
            (widgetData.props?.cancelText as string) ||
            t("editor.dialog.cancel")
          }}</el-button>
          <el-button type="primary" @click="handleConfirm">{{
            (widgetData.props?.confirmText as string) ||
