...
describe("FgPieChart", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_pie_chart")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_pie_chart")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("pie-chart", "test_pie_chart")!;
      store.addWidget(widget);
      store.updateWidget("test_pie_chart", {
        props: { nameField: "label", valueField: "count", title: "Pie" },
      });
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_pie_chart")!.events).toHaveLength(1);
      expect(store.findWidget("test_pie_chart")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
  ...
  describe("数据源", () => {
    it("支持 api 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_pie_chart")!.api!.url).toBe(
        "/api/pie-data",
      );
    });
  });
