...
describe("FgStatistic", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_statistic")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_statistic")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("API 数据源", () => {
    it("支持 apiUrl 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_statistic")!.props!.apiUrl).toBe(
        "/api/stat",
      );
    });
    ...
    it("responseDataPath 可配置", () => {
      ...
      widget.props = {
        ...widget.props,
        apiUrl: "/api/stat",
        responseDataPath: "data.total",
      };
      store.addWidget(widget);
      expect(store.findWidget("test_statistic")!.props!.responseDataPath).toBe(
        "data.total",
      );
    });
  });
  ...
  describe("趋势展示", () => {
    it("trendValue 为空时不渲染趋势区域", () => {
      ...
      const footer = wrapper
        .findAll("div")
        .find((d) => d.text().includes("+12%"));
      expect(footer).toBeUndefined();
    });

    it("trend=up 且 trendValue 非空时渲染趋势", () => {
      const wrapper = mountStatistic({
        props: { trend: "up", trendValue: "+12%" },
      });
