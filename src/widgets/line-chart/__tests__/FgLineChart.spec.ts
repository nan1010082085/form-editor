...
describe("FgLineChart", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_line_chart")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_line_chart")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("line-chart", "test_line_chart")!;
      store.addWidget(widget);
      store.updateWidget("test_line_chart", {
        props: { xField: "date", yField: "amount", title: "Trend" },
      });
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_line_chart")!.events).toHaveLength(1);
      expect(store.findWidget("test_line_chart")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
  ...
  describe("数据源", () => {
    it("支持 api 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_line_chart")!.api!.url).toBe(
        "/api/line-data",
      );
    });
  });
