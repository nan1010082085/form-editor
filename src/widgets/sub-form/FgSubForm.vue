...
const fields = computed(
  () =>
    (widgetData.value.props?.fields as Array<{
      key: string;
      label: string;
      type: string;
      placeholder?: string;
...
const minRows = computed(
  () => (widgetData.value.props?.minRows as number) ?? 0,
);
const maxRows = computed(
  () => (widgetData.value.props?.maxRows as number) ?? 100,
);
const addButtonText = computed(
  () =>
    (widgetData.value.props?.addButtonText as string) ??
    t("editor.subForm.addRow"),
);
  ...
  <div :class="styles.subForm" :style="widgetStyle">
    ...
    <template v-else>
      <div :class="styles.header">
        ...
        <div :class="styles.headerAction">
          {{ t("editor.subForm.actionHeader") }}
        </div>
      </div>
