...
describe("FgFunnel", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_funnel")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_funnel")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("funnel", "test_funnel")!;
      store.addWidget(widget);
      store.updateWidget("test_funnel", {
        props: { nameField: "stage", valueField: "count", title: "Funnel" },
      });
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_funnel")!.events).toHaveLength(1);
      expect(store.findWidget("test_funnel")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
  ...
  describe("数据源", () => {
    it("支持 api 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_funnel")!.api!.url).toBe(
        "/api/funnel-data",
      );
    });
  });
