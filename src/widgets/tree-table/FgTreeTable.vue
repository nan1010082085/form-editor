...
const apiUrl = computed(() =>
  api.value?.url ? resolveWidgetUrl(api.value.url, variablesContext.value) : "",
);
...
const {
  ...
  fetcher: () =>
    fetchWidgetDataSource<unknown>(apiUrl.value).then(normalizeTree),
  enabled: computed(
    () =>
...
const rowKey = computed(
  () => (widgetData.value.props?.rowKey as string) || "id",
);
const childrenKey = computed(
  () => (widgetData.value.props?.childrenKey as string) || "children",
);
const defaultExpandAll = computed(
  () => widgetData.value.props?.defaultExpandAll !== false,
);
const stripe = computed(() => widgetData.value.props?.stripe !== false);
const border = computed(() => widgetData.value.props?.border !== false);
const tableHeight = computed(
  () => (widgetData.value.props?.height as number) ?? 560,
);
