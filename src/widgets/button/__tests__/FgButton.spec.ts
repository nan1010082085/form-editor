...
describe("FgButton", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_widget")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_widget")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("store CRUD", () => {
    ...
    it("更新 props 后 store 中同步", () => {
      const widget = createWidget("button", "test_btn")!;
      store.addWidget(widget);
      store.updateWidget("test_btn", {
        props: { ...widget.props, text: "提交" },
      });
  ...
  describe("button type", () => {
    it("默认按钮类型为 primary", () => {
      ...
      expect(wrapper.find(".el-button").classes()).toContain(
        "el-button--primary",
      );
    });
  ...
  describe("事件", () => {
    it("支持 click 事件配置", () => {
      const widget = createWidget("button", "test_widget")!;
      widget.events = [
        { trigger: "click", actions: [{ type: "show", target: "w2" }] },
      ];
      store.addWidget(widget);
    ...
    it("支持多个 click action", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.events![0].actions).toHaveLength(
        3,
      );
    });
      ...
      widget.events = [
        {
          trigger: "click",
          confirm: "确认提交？",
          actions: [{ type: "submit", target: "" }],
        },
      ];
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.events![0].confirm).toBe(
        "确认提交？",
      );
    });
  });
  ...
  describe("样式", () => {
    it("支持 backgroundColor 样式", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.style!.backgroundColor).toBe(
        "#ff0000",
      );
    });
  });
