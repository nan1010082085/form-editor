...
describe("FgDivider", () => {
  ...
  describe("store CRUD", () => {
    ...
    it("更新 direction 后 store 中同步", () => {
      const widget = createWidget("divider", "test_div")!;
      store.addWidget(widget);
      store.updateWidget("test_div", {
        props: { ...widget.props, direction: "vertical" },
      });
  ...
  describe("direction 属性", () => {
    it("默认 direction=horizontal", () => {
      const widget = createWidget("divider", "test_widget")!;
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.props!.direction).toBe(
        "horizontal",
      );
    });
  ...
  describe("contentPosition 属性", () => {
    it("默认 contentPosition=center", () => {
      const widget = createWidget("divider", "test_widget")!;
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.props!.contentPosition).toBe(
        "center",
      );
    });
