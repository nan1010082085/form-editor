...
const cardTitleField = computed(() =>
  String(widgetData.value.props?.cardTitleField ?? "title"),
);
const cardSubtitleField = computed(() => {
  const v = widgetData.value.props?.cardSubtitleField;
  return v ? String(v) : "";
});
const statusField = computed(() =>
  String(widgetData.value.props?.statusField ?? "status"),
);
...
const apiUrl = computed(() =>
  api.value?.url
    ? resolveWidgetUrl(`${api.value.url}?page=1&pageSize=100`, {})
    : "",
);
...
const {
  ...
  fetcher: () =>
    fetchWidgetDataSource<unknown>(apiUrl.value).then(normalizeList),
  enabled: computed(
    () =>
      !!apiUrl.value && !shouldUseWidgetMock(surface, Boolean(apiUrl.value)),
  ),
  retry: 2,
  swr: false,
  cacheTtl: 0,
      ...
      >
        ...
        <div :class="styles.cardList">
          ...
          >
            ...
            <div
              v-if="cardSubtitleField && card[cardSubtitleField]"
              :class="styles.cardSub"
            >
