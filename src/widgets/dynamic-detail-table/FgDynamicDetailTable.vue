...
function sumAmountColumn(): number {
  const amountCol = columns.value.find(
    (c) => c.prop === "amount" || c.type === "number",
  );
  if (!amountCol) return 0;
  return rows.value.reduce(
    (sum, row) => sum + (Number(row[amountCol.prop]) || 0),
    0,
  );
}
...
const title = computed(
  () =>
    ...
    t("editor.dynamicDetailTable.title"),
);
const columns = computed<DetailColumn[]>(
  () =>
    (widgetData.value.props?.columns as DetailColumn[]) ??
    dynamicDetailTableMock.defaultProps.columns,
);
  ...
  <div :class="styles.wrapper">
    <div :class="styles.header">
      ...
      <el-button type="primary" size="small" @click="addRow">{{
        t("editor.dynamicDetailTable.addRow")
      }}</el-button>
    </div>
    <el-table :data="rows" border size="small" :class="styles.table">
      ...
      <el-table-column
        :label="t('editor.dynamicDetailTable.actions')"
        width="80"
        fixed="right"
      >
        <template #default="{ $index }">
          <el-button
            ...
            >{{ t("editor.dynamicDetailTable.delete") }}</el-button
          >
        </template>
      </el-table-column>
    </el-table>
