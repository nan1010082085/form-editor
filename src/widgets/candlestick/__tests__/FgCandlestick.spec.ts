...
describe("FgCandlestick", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_candlestick")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_candlestick")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("candlestick", "test_candlestick")!;
      store.addWidget(widget);
      store.updateWidget("test_candlestick", {
        props: { dateField: "time", title: "K-line" },
      });
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_candlestick")!.events).toHaveLength(1);
      expect(store.findWidget("test_candlestick")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
  ...
  describe("数据源", () => {
    it("支持 api 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_candlestick")!.api!.url).toBe(
