...
describe("FgHeatmap", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_heatmap")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_heatmap")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("heatmap", "test_heatmap")!;
      store.addWidget(widget);
      store.updateWidget("test_heatmap", {
        props: {
          xField: "col",
          yField: "row",
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_heatmap")!.events).toHaveLength(1);
      expect(store.findWidget("test_heatmap")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
  ...
  describe("数据源", () => {
    it("支持 api 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_heatmap")!.api!.url).toBe(
        "/api/heatmap-data",
      );
    });
  });
