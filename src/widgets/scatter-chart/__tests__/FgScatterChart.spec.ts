...
describe("FgScatterChart", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_scatter_chart")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_scatter_chart")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("scatter-chart", "test_scatter_chart")!;
      store.addWidget(widget);
      store.updateWidget("test_scatter_chart", {
        props: { xField: "price", yField: "sales", title: "Scatter" },
      });
  ...
  describe("事件", () => {
    ...
    it("支持 set-data 事件配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_scatter_chart")!.events).toHaveLength(1);
      expect(store.findWidget("test_scatter_chart")!.events![0].trigger).toBe(
        "set-data",
      );
    });
  });
  ...
  describe("数据源", () => {
    it("支持 api 配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_scatter_chart")!.api!.url).toBe(
        "/api/scatter-data",
      );
    });
  });
