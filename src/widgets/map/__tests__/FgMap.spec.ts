...
describe("FgMap", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_map")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_map")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("属性", () => {
    ...
    it("props 可覆盖", () => {
      const widget = createWidget("map", "test_map")!;
      store.addWidget(widget);
      store.updateWidget("test_map", {
        props: { mapType: "world", showLabel: false },
      });
  ...
  describe("事件", () => {
    it("支持 chart-click 事件目标", () => {
      const widget = createWidget("map", "test_map")!;
      widget.events = [
        { trigger: "chart-click", actions: [{ type: "show", target: "w2" }] },
      ];
      store.addWidget(widget);
