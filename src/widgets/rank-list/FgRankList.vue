...
const items = computed(
  () => (widgetData.value.props?.items as Array<Record<string, unknown>>) ?? [],
);
const nameKey = computed(
  () => (widgetData.value.props?.nameKey as string) ?? "name",
);
const valueKey = computed(
  () => (widgetData.value.props?.valueKey as string) ?? "value",
);
const trendKey = computed(
  () => (widgetData.value.props?.trendKey as string) ?? "trend",
);
const maxItems = computed(
  () => (widgetData.value.props?.maxItems as number) ?? 10,
);
...
const highlightTop = computed(
  () => (widgetData.value.props?.highlightTop as number) ?? 3,
);
...
const sortedItems = computed(() => {
  return [...items.value]
    .sort(
      (a, b) =>
        ((b[valueKey.value] as number) ?? 0) -
        ((a[valueKey.value] as number) ?? 0),
    )
  ...
  <div :class="styles.rankList" :style="widgetStyle">
    ...
    <div :class="styles.list">
      <div
        ...
        :class="styles.item"
      >
        <span v-if="showRank" :class="[styles.rank, getRankClass(index)]">{{
          index + 1
        }}</span>
        <span :class="styles.name">{{ item[nameKey] ?? "" }}</span>
        <span :class="styles.value">{{ item[valueKey] ?? "" }}</span>
        <span
          v-if="showTrend && item[trendKey] !== undefined"
          :class="[styles.trend, getTrendClass(item[trendKey])]"
        >
          {{ getTrendIcon(item[trendKey]) }}
        </span>
      </div>
      <div v-if="sortedItems.length === 0" :class="styles.empty">
        {{ t("editor.rankList.empty") }}
      </div>
    </div>
  </div>
</template>
