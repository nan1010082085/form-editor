...
const showWeekday = computed(
  () => widgetData.value.props?.showWeekday === true,
);
const format = computed(
  () => (widgetData.value.props?.format as string) ?? "24h",
);
const dateFormat = computed(
  () => (widgetData.value.props?.dateFormat as string) ?? "YYYY-MM-DD",
);
...
const weekdayStr = computed(() =>
  t(`editor.realtimeClock.${weekdayKeys[now.value.getDay()]}`),
);
