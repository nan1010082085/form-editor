...
const visibleColumns = computed(() => {
  return tableColumns.value
    .map((key) => {
      ...
      return i18nKey
        ? { ...col, label: t(`editor.userManagement.${i18nKey}`) }
        : col;
    })
    .filter(Boolean);
});
...
const formRules = computed(() => ({
  username: [
    {
      required: true,
      message: t("editor.userManagement.enterUsername"),
      trigger: "blur",
    },
  ],
  password: [
    {
      required: true,
      message: t("editor.userManagement.enterPassword"),
      trigger: "blur",
    },
  ],
  displayName: [
    {
      required: true,
      message: t("editor.userManagement.enterNickname"),
      trigger: "blur",
    },
  ],
}));
...
const resetPwdRules = computed(() => ({
  password: [
    {
      required: true,
      message: t("editor.userManagement.enterNewPassword"),
      trigger: "blur",
    },
    {
      min: 6,
      message: t("editor.userManagement.passwordMinLength"),
      trigger: "blur",
    },
  ],
}));
...
function applyEditorMock(): boolean {
  ...
  tableData.value = mock.rows as unknown as UserItem[];
...
async function handleDelete(row: UserItem) {
  try {
    await ElMessageBox.confirm(
      t("editor.userManagement.confirmDeleteUser"),
  ...
  <div :class="styles.container">
    ...
    <div :class="styles.tableWrap">
      ...
      >
        ...
        >
          <template #default="{ row }">
            ...
            >
              {{
                row.status === "active"
                  ? t("editor.userManagement.enabled")
                  : t("editor.userManagement.disabled")
              }}
            </el-tag>
        ...
        <el-table-column
          :label="t('editor.userManagement.actions')"
          width="220"
          fixed="right"
        >
          <template #default="{ row }">
    ...
    <el-dialog
      ...
      :title="
        dialogMode === 'add'
          ? t('editor.userManagement.addUserTitle')
          : t('editor.userManagement.editUserTitle')
      "
      width="560px"
      destroy-on-close
    >
      <el-form
        ...
        :rules="
          dialogMode === 'add'
            ? formRules
            : { ...formRules, password: undefined }
        "
        label-width="80px"
        :class="styles.formGrid"
      >
        <el-form-item
          :label="t('editor.userManagement.col_username')"
          prop="username"
        ...
        <el-form-item
          :label="t('editor.userManagement.col_displayName')"
          prop="displayName"
        >
          <el-input
            v-model="formData.displayName"
            :placeholder="t('editor.userManagement.enterNickname')"
          />
        </el-form-item>
        <el-form-item
          :label="t('editor.userManagement.col_phone')"
          prop="phone"
        >
          <el-input
            v-model="formData.phone"
            :placeholder="t('editor.userManagement.enterPhone')"
          />
        </el-form-item>
        <el-form-item :label="t('editor.userManagement.email')" prop="email">
          <el-input
            v-model="formData.email"
            :placeholder="t('editor.userManagement.enterEmail')"
          />
        </el-form-item>
        <el-form-item
          :label="t('editor.userManagement.col_deptId')"
          prop="deptId"
        >
          <el-input
            v-model="formData.deptId"
            :placeholder="t('editor.userManagement.enterDepartment')"
          />
        </el-form-item>
        <el-form-item
          :label="t('editor.userManagement.col_status')"
          prop="status"
          :class="styles.fullWidth"
        >
          <el-select
            v-model="formData.status"
            :placeholder="t('editor.userManagement.selectStatus')"
          >
            <el-option
              :label="t('editor.userManagement.enabled')"
              value="active"
            />
            <el-option
              :label="t('editor.userManagement.disabled')"
              value="inactive"
            />
          </el-select>
        </el-form-item>
    ...
    >
      ...
      >
        <el-form-item :label="t('editor.userManagement.user')">
          <span
            >{{ resetPwdTarget?.displayName }}（{{
              resetPwdTarget?.username
            }}）</span
          >
        </el-form-item>
        <el-form-item
          :label="t('editor.userManagement.newPassword')"
          prop="password"
        >
          <el-input
