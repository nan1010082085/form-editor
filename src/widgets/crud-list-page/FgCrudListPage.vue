...
const detailDialogConfig = computed<CrudDetailDialogConfig | null>(() => {
  const raw = widgetData.value.props?.detailDialog as
    | CrudDetailDialogConfig
    | undefined;
...
const formDialogConfig = computed<CrudFormDialogConfig | null>(() => {
  const raw = widgetData.value.props?.formDialog as
    | CrudFormDialogConfig
    | undefined;
...
const formDialogTitle = computed(() => {
  ...
  if (formMode.value === "add")
    return cfg.createTitle ?? cfg.title ?? t("editor.crudListPage.createTitle");
...
async function submitFormDialog() {
  ...
  try {
    if (isEditorSurface.value) {
      ElMessage.success(
        formMode.value === "add"
          ? t("editor.crudListPage.previewCreateSuccess")
          : t("editor.crudListPage.previewSaveSuccess"),
      );
...
async function loadTimeline() {
  ...
  const instanceId = String(
    detailRow.value.flowInstanceId ?? detailData.value.flowInstanceId ?? "",
  );
  if (!instanceId) {
    timelineLogs.value = [];
    return;
    ...
    >
      <div v-loading="detailLoading" :class="styles.detailBody">
        <el-descriptions
          ...
          :title="
            detailDialogConfig.title || t('editor.crudListPage.applyInfo')
          "
          :column="2"
          border
        >
        ...
        >
          <div :class="styles.timelineTitle">
            {{ t("editor.crudListPage.approvalRecord") }}
          </div>
          <div v-if="timelineLoading" :class="styles.timelineLoading">
            {{ t("editor.crudListPage.loading") }}
          </div>
          ...
          <div v-else :class="styles.timelineEmpty">
            {{ t("editor.crudListPage.noApprovalRecord") }}
          </div>
        </div>
      </div>
      ...
      <template #footer>
        ...
        >
          {{
            detailDialogConfig.confirmText ||
            t("editor.crudListPage.fullscreenApproval")
          }}
        </el-button>
      </template>
    </el-dialog>
