...
describe("FgDateTimeSlot", () => {
  ...
  describe("Store CRUD", () => {
    ...
    it("可更新属性", () => {
      const widget = createWidget("date-time-slot", "test_widget")!;
      store.addWidget(widget);
      store.updateWidget("test_widget", {
        props: { startPlaceholder: "开始", endPlaceholder: "结束" },
      });
      expect(store.findWidget("test_widget")!.props!.startPlaceholder).toBe(
        "开始",
      );
      expect(store.findWidget("test_widget")!.props!.endPlaceholder).toBe(
        "结束",
      );
    });
  ...
  describe("属性", () => {
    ...
    it("format 生效", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_widget")!.props!.format).toBe(
        "YYYY/MM/DD HH:mm",
      );
    });
