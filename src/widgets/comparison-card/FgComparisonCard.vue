...
const previousValue = computed(
  () => widgetData.value.props?.previousValue ?? 0,
);
...
const comparisonLabel = computed(
  () =>
    (widgetData.value.props?.comparisonLabel as string) ??
    t("editor.comparisonCard.defaultLabel"),
);
const precision = computed(
  () => (widgetData.value.props?.precision as number) ?? 1,
);
...
const formattedValue = computed(() => {
  ...
  if (v >= 100000000)
    return `${(v / 100000000).toFixed(precision.value)}${t("editor.comparisonCard.unitYi")}`;
  if (v >= 10000)
    return `${(v / 10000).toFixed(precision.value)}${t("editor.comparisonCard.unitWan")}`;
