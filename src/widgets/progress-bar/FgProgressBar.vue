...
const variant = computed(
  () => (widgetData.value.props?.variant as string) ?? "line",
);
const strokeWidth = computed(
  () => (widgetData.value.props?.strokeWidth as number) ?? 6,
);
const showText = computed(() => widgetData.value.props?.showText !== false);
const format = computed(
  () => (widgetData.value.props?.format as string) ?? "percent",
);
const color = computed(() => (widgetData.value.props?.color as string) ?? "");
const thresholds = computed(
  () =>
    (widgetData.value.props?.thresholds as Array<{
      value: number;
      color: string;
    }>) ?? [],
);
...
<template>
  <div :class="styles.progressBar" :style="widgetStyle">
    ...
    <template v-else>
      <div
        :class="styles.circleOuter"
        :style="{
          width: `${widgetData.props?.size ?? 120}px`,
          height: `${widgetData.props?.size ?? 120}px`,
        }"
      >
        ...
        <span v-if="showText" :class="styles.circleText">{{
          displayText
        }}</span>
      </div>
    </template>
