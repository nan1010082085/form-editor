...
const height = computed(
  () => (widgetData.value.props?.height as string) || "100%",
);
const variables = computed(
  () => (widgetData.value.props?.variables as Record<string, unknown>) ?? {},
);
...
async function loadApp() {
  ...
  timeoutTimer = setTimeout(() => {
    if (loading.value) {
      error.value = t("editor.microAppContainer.loadTimeout", {
        ms: timeout.value,
      });
      loading.value = false;
  ...
  <div :class="styles.container" :style="{ height }">
    <div v-if="isEditorSurface" :class="styles.preview">
      ...
      <span :class="styles.hint">{{
        t("editor.microAppContainer.editorHint")
      }}</span>
    </div>
