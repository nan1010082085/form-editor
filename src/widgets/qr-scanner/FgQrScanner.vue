...
const label = computed(
  () =>
    (widgetData.value.props?.label as string) || t("editor.qrScanner.label"),
);
const placeholder = computed(
  () =>
    (widgetData.value.props?.placeholder as string) ||
    t("editor.qrScanner.placeholder"),
);
...
<template>
  <div :class="styles.wrapper">
    ...
    <div :class="styles.row">
      ...
      />
      <el-button type="primary" @click="simulateScan">{{
        t("editor.qrScanner.simulateScan")
      }}</el-button>
    </div>
    <p v-if="field" :class="styles.hint">
      {{ t("editor.qrScanner.boundField", { field }) }}
    </p>
  </div>
</template>
