...
const title = computed(
  () =>
    ...
    t("editor.flowTaskActions.defaultTitle"),
);
const canAct = computed(
  () => task.value && ["pending", "claimed"].includes(task.value.status),
);
...
function resolveTaskId(): string {
  const varName =
    (widgetData.value.props?.taskIdVariable as string) || "taskId";
  ...
  try {
    ...
    ElMessage.error(
      err instanceof Error
        ? err.message
        : t("editor.flowTaskActions.approveFailed"),
    );
  } finally {
    acting.value = false;
  }
...
async function handleReject() {
  ...
  try {
    ...
    ElMessage.error(
      err instanceof Error
        ? err.message
        : t("editor.flowTaskActions.rejectFailed"),
    );
  } finally {
    acting.value = false;
  }
...
async function handleClaim() {
  ...
  try {
    ...
    ElMessage.error(
      err instanceof Error
        ? err.message
        : t("editor.flowTaskActions.claimFailed"),
    );
  } finally {
    acting.value = false;
  }
...
async function handleDelegate() {
  ...
  try {
    ...
    ElMessage.error(
      err instanceof Error
        ? err.message
        : t("editor.flowTaskActions.delegateFailed"),
    );
  } finally {
    acting.value = false;
  }
...
<template>
  <div :class="styles.wrapper">
    ...
    <div v-if="surface === 'editor'" :class="styles.editorBadge">
      {{ t("editor.flowTaskActions.editorPreview") }}
    </div>

    <div v-if="loading" :class="styles.loading">
      {{ t("editor.flowTaskActions.loadingTask") }}
    </div>
    <div v-else-if="!task" :class="styles.empty">
      {{ t("editor.flowTaskActions.noPendingTask") }}
    </div>
    <template v-else>
      <div v-if="aiSuggestion" :class="styles.suggestion">
        <el-alert
          ...
          :closable="false"
        />
        <el-button size="small" type="primary" link @click="applySuggestion">{{
          t("editor.flowTaskActions.adoptSuggestion")
        }}</el-button>
      </div>

      <div v-if="canAct" :class="styles.actions">
        <el-button type="success" :loading="acting" @click="handleApprove">{{
          t("editor.flowTaskActions.approve")
        }}</el-button>
        <el-button type="danger" :loading="acting" @click="showReject = true">{{
          t("editor.flowTaskActions.reject")
        }}</el-button>
        <el-button
          v-if="task.status === 'pending'"
          :loading="acting"
          @click="handleClaim"
          >{{ t("editor.flowTaskActions.claim") }}</el-button
        >
        <el-button :loading="acting" @click="showDelegate = true">{{
          t("editor.flowTaskActions.delegate")
        }}</el-button>
      </div>
      <div v-else :class="styles.done">
        {{ t("editor.flowTaskActions.taskProcessed") }}
      </div>
    </template>

    <el-dialog
      v-model="showReject"
      :title="t('editor.flowTaskActions.rejectTitle')"
      width="420px"
    >
      <el-form label-width="80px">
        <el-form-item :label="t('editor.flowTaskActions.rejectTo')">
          <el-select
            ...
            :placeholder="t('editor.flowTaskActions.selectNode')"
          >
            <el-option
      ...
      <template #footer>
        ...
        <el-button type="danger" :loading="acting" @click="handleReject">{{
          t("editor.flowTaskActions.confirmReject")
        }}</el-button>
      </template>
    </el-dialog>

    <el-dialog
      v-model="showDelegate"
      :title="t('editor.flowTaskActions.delegateTitle')"
      width="420px"
    >
      <el-form label-width="80px">
        <el-form-item :label="t('editor.flowTaskActions.delegateTo')">
          <el-input
            ...
            :placeholder="t('editor.flowTaskActions.userId')"
          />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showDelegate = false">{{
          t("editor.common.cancel")
        }}</el-button>
        <el-button type="primary" :loading="acting" @click="handleDelegate">{{
          t("editor.flowTaskActions.confirmDelegate")
        }}</el-button>
      </template>
    </el-dialog>
  </div>
