...
const exposedContext = inject<Ref<
  Record<string, Record<string, unknown>>
> | null>("exposedContext", null);
...
<template>
  <div v-if="showStatus" :class="styles.container">
    ...
    <span v-if="surface === 'editor'">{{
      t("editor.autoRefresh.previewStatus", { interval: intervalSeconds })
    }}</span>
    <span v-else>{{
      t("editor.autoRefresh.countdownStatus", { countdown })
    }}</span>
