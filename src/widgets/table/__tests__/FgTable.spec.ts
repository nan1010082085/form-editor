...
describe("FgTable", () => {
      ...
      global: {
        ...
        provide: {
          [widgetDataKey as symbol]: computed(
            () => store.findWidget("test_table")!,
          ),
          [widgetStyleKey as symbol]: computed(
            () => store.findWidget("test_table")!.style ?? {},
          ),
        },
      },
    });
  ...
  describe("Props", () => {
    it("默认 columns 配置包含两个列", () => {
      const widget = createWidget("table", "test_table")!;
      store.addWidget(widget);
      const columns = widget.props?.columns as Array<{
        prop: string;
        label: string;
      }>;
    ...
    it("渲染正确的列数", () => {
      const widget = createWidget("table", "test_table")!;
      store.addWidget(widget);
      const columns = store.findWidget("test_table")!.props!.columns as Array<{
        prop: string;
      }>;
      expect(columns).toHaveLength(2);
    });
  ...
  describe("API 数据源", () => {
    ...
    it("props 中 apiUrl 可配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_table")!.props!.apiUrl).toBe(
        "/api/table-list",
      );
    });
    ...
    it("props 中 responseDataPath 可配置", () => {
      ...
      store.addWidget(widget);
      expect(store.findWidget("test_table")!.props!.responseDataPath).toBe(
        "result.records",
      );
    });
  });
  ...
  describe("分页", () => {
    it("默认 pagination.enabled 为 true", () => {
      const widget = createWidget("table", "test_table")!;
      store.addWidget(widget);
      const pagination = widget.props?.pagination as {
        enabled: boolean;
        pageSize: number;
      };
    ...
    it("pagination 可配置为禁用", () => {
      ...
      widget.props = {
        ...widget.props,
        pagination: { enabled: false, pageSize: 10, pageSizes: [10, 20] },
      };
      store.addWidget(widget);
      const pagination = store.findWidget("test_table")!.props!.pagination as {
        enabled: boolean;
      };
  ...
  describe("行选择", () => {
    ...
    it("selection 可配置为启用", () => {
      ...
      store.addWidget(widget);
      const selection = store.findWidget("test_table")!.props!.selection as {
        enabled: boolean;
      };
    ...
    it("启用 selection 时渲染选择列", () => {
      ...
      expect((widget.props?.selection as { enabled: boolean }).enabled).toBe(
        true,
      );
    });
